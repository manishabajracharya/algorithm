...
public static StringBuilder get_string(String str, int first){
...
for(int i=0;i<str.length()-1;i++){
    if(i-1<0){
    n=Character.getNumericValue(str.charAt(i))-Character.getNumericValue(st.charAt(i));
    }
    else{
    n = Character.getNumericValue(str.charAt(i))-Character.getNumericValue(st.charAt(i-1))-Character.getNumericValue(st.charAt(i));
    }
    
    if(n>1){
    st.replace(0, st.length(), "NONE");
    break;
    }
    else{    
    st.append(n);
...
st[0]=get_string(str,0).toString();

st[1]=get_string(str,1).toString();
 ...
 public static void main(String[] args)
        {
            ...
            System.out.println(Arrays.toString(decode(str)));
